...
template <class T>
void print(vector<NAMESPACE::vector<T> > &lst) {
  for (typename NAMESPACE::vector<NAMESPACE::vector<T> >::iterator it =
           lst.begin();
       it != lst.end(); it++) {
    for (typename NAMESPACE::vector<T>::iterator it2 = it->begin();
         it2 != it->end(); it2++)
      std::cout << *it2 << ' ';
    std::cout << '\n';
  }
...
void print(vector<T> &lst) {
  for (typename NAMESPACE::vector<T>::iterator it = lst.begin();
       it != lst.end(); it++)
    std::cout << *it << ' ';
  std::cout << '\n';
}
...
void print_vector(vector<T> &test) {
  ...
  for (typename NAMESPACE::vector<T>::iterator it = beg; it != end; it++) {
    std::cout << *it << " ";
    if (((it - beg) % 10 == 9) && it > beg) std::cout << std::endl;
  }
  std::cout << std::endl;
}
...
void push_pop_back_tests(void) {
  ...
  std::cout << "Empty ? " << test.empty() << " / Capacity : " << test.capacity()
...
void reverse_it_tests(void) {
  ...
  for (typename NAMESPACE::vector<T>::reverse_iterator it = revbeg;
       it != test.rend(); it++) {
    std::cout << *it << " ";
    if (!((revbeg - it) % 10) && it != revbeg) std::cout << std::endl;
  }
