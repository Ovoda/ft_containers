...
class red_black_tree {
 ...
 private:
  ...
  void print2DUtil(node *curr, int space) {
    ...
    if (curr->color == RED) {
      std::cout << "\033[1;31m " << curr->data.first << " [" << curr->parent
                << "]"
                << "\033[0m\n";
    } else {
      std::cout << curr->data.first << " [" << curr->parent << "]"
                << "\n";
    }
  ...
  void delete_node_helper(node_ptr curr, typename data_type::first_type key) {
    ...
    _size--;
    if (color_anchor == BLACK && _size > 0 && !node_anchor->is_leaf) {
  ...
  void reset_root() {
    ...
    if (_root->is_leaf) {
      _alloc.construct(
          _end, create_node(data_type(), _root, _root, _root, BLACK, true));
    } else {
      _alloc.construct(_end, create_node(maximum(_root)->data, _root, _root,
                                         _root, BLACK, true));
    }
    _root->parent = _end;
  }
